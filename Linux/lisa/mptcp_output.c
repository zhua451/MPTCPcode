...
int flownum;
flownum=flow[ntohs(((struct inet_sock *)tp)->inet_dport)];
	struct mptcp_cb *mpcb = tp->mpcb;
	struct sock *sk;

if(!(lisaFlag[flownum]<0)){
	if(tp->mptcp->path_index==2){
	    u32 old_cwnd;
	    mptcp_for_each_sk(mpcb, sk) {
    		struct tcp_sock *tp_it = tcp_sk(sk);
	  	if(tp_it->mptcp->path_index==1 && tp_it->snd_cwnd>20){
		   tp_it->snd_cwnd-=10;
		   tp->snd_cwnd=10;

		}else if(tp_it->mptcp->path_index==1 && tp_it->snd_cwnd>6){
		   tp->snd_cwnd=(tp_it->snd_cwnd)/2;		   
		   tp_it->snd_cwnd-=tp->snd_cwnd;
...
int mptcp_write_wakeup(struct sock *meta_sk)
{
	...
	if (meta_sk->sk_state == TCP_CLOSE)
		return -1;
//printk("mptcp_write_wakeup\n");
	skb = tcp_send_head(meta_sk);
//if(skb)printk("skb is not null\n");
	if (skb &&
...
//printk("mptcp_output.c subflowId:%d\n",(int)(tcp_sk(subsk)->mptcp->path_index));
...
int flownum;
flownum=flow[ntohs(((struct inet_sock *)meta_tp)->inet_dport)];
struct timeval tv;
do_gettimeofday(&(tv));
	mptcp_for_each_sk(mpcb, sk) {
			struct tcp_sock *tp = tcp_sk(sk);
			if (flownum % 20 == 0){
				printk("mptcp_output.c 1 flownum: %d sFlowId: %d cwnd: %u rtt: %u sec: %d usec: %d\n",flownum,tp->mptcp->path_index,tp->snd_cwnd,tp->srtt,tv.tv_sec,tv.tv_usec);
			}
...
{
//printk("mptcp_output.c timeout test!!\n");
	struct tcp_sock *meta_tp = tcp_sk(meta_sk);
	struct mptcp_cb *mpcb = meta_tp->mpcb;
