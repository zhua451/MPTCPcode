...
EXPORT_SYMBOL(flow);
u32 baseRtt[10000];
EXPORT_SYMBOL(baseRtt);
int num;
int seconds[10000];
int useconds[10000];
EXPORT_SYMBOL(seconds);
EXPORT_SYMBOL(useconds);
...
{	
	...
	if(mpcb->cnt_subflows==1){
	   printk("#\n");
	   flowNum++;
	   flow[ntohs(((struct inet_sock *)tp)->inet_dport)]=flowNum;
	   baseRtt[flowNum]=65535;
	   if(seconds[flowNum]==0 && useconds[flowNum]==0){
	    	seconds[flowNum] = tv.tv_sec;
	    	useconds[flowNum] = tv.tv_usec;
		printk("mptcp_ipv4.c flownum: %d seconds: %d useconds: %d\n",flowNum,seconds[flowNum],useconds[flowNum]);
	   }
	}
	if(tp->mptcp->path_index==2){
	   ...
	   mptcp_for_each_sk(mpcb, sk) {
	  	...
	  	if(tp_it->mptcp->path_index==1)
		   num=flow[ntohs(((struct inet_sock *)tp_it)->inet_dport)];
	   }
	   flow[ntohs(((struct inet_sock *)tp)->inet_dport)]=num;
	}
/*printk("mptcp_ctrl.c %s: token %#x pi %d, src_addr:%pI4:%d dst_addr:%pI4:%d, cnt_subflows now %d\n",
			    __func__ , mpcb->mptcp_loc_token,
			    tp->mptcp->path_index,
			    &((struct inet_sock *)tp)->inet_saddr,
			    ntohs(((struct inet_sock *)tp)->inet_sport),
	...
	if(tp->mptcp->path_index==2){
	   ...
	   flow[ntohs(((struct inet_sock *)tp)->inet_dport)]=num;
		...
		/*printk("mptcp_ctrl.c  %s: token %#x pi %d, src_addr:%pI6:%d dst_addr:%pI6:%d, cnt_subflows now %d\n",
			    __func__ , mpcb->mptcp_loc_token,
			    tp->mptcp->path_index, &inet6_sk(sk)->saddr,
			    ntohs(((struct inet_sock *)tp)->inet_sport),
			    &sk->sk_v6_daddr,
