...
MpTcpSocketBase::MpTcpSocketBase (void)
{
	...
	NS_LOG_FUNCTION(this);
	    ...
	    m_algoPR = NoPR_Algo;
           // m_algoLL = No_Slow_StartSubflowAlgo;
                
            //m_algoSS = Slow_Start_RR;
...
InitialRtt(sFlow->routeId);
/*switch(sFlow->routeId){
           case 0:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 1:sfp[sFlow->routeId].rtt=0.1;
                 sfr[sFlow->routeId]=0.1;break;
           case 2:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 3:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 4:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 5:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 6:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
           case 7:sfp[sFlow->routeId].rtt=0.1;
                  sfr[sFlow->routeId]=0.1;break;
        }*/
...
//std::cout<<"DupAck  m_segmentSize    "<< m_segmentSize<<"BytesInFlight ()  "<<BytesInFlight ()<<"BytesInFlight () / 2    "<<BytesInFlight () / 2<<"m_ssThresh  "<<m_ssThresh<<std::endl;
...
//std::cout<<"ReTxTimeout     sFlow->ssthresh:"<<sFlow->ssthresh<<"    Second:"<<Simulator::Now().GetSeconds()<<std::endl;
//std::cout<<"reduceCWND (sFlowIdx)  sFlow->cwnd :"<<sFlow->cwnd<<std::endl;
  reduceCWND (sFlowIdx);
 

     
//std::cout<<"ReTxTimeout  sFlowIdx:"<<(int)sFlowIdx<<"  flow:"<<sFlow->dPort-5000<<"   reduceCWND (sFlowIdx) Second:"<<Simulator::Now().GetSeconds()<<"   sFlow->cwnd"<<sFlow->cwnd <<"     sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<std::endl;
//std::cout<<"ReTxTimeout  sFlowIdx:"<<(int)sFlowIdx<<"  flow"<<sFlow->dPort-5000<<"   reduceCWND (sFlowIdx)"<<"    Second:"<<Simulator::Now().GetSeconds()<<"   sFlow->cwnd"<<sFlow->cwnd <<"     sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<std::endl;
...
//std::cout<<"default: sFlow->ssthresh  "<<sFlow->ssthresh<<std::endl;
            break;
    }


   sFlow->ssthresh = (std::min(remoteRecvWnd, static_cast<uint32_t>(sFlow->cwnd)*sFlow->MSS)) / 2; // Per RFC2581
//std::cout<<"sFlowIdx:"<<(int)sFlowIdx<<"   reduceCWND  ssthresh 1   "<<"remoteRecvWnd:   "<<remoteRecvWnd<<"static_cast<uint32_t>(sFlow->cwnd):    "<<static_cast<uint32_t>(sFlow->cwnd)<<"   sFlow->ssthresh:"<<sFlow->ssthresh<<std::endl;
    sFlow->ssthresh = std::max (sFlow->ssthresh, 2 * sFlow->MSS);
//std::cout<<"reduceCWND  ssthresh 2   "<<"sFlow->MSS:  "<<sFlow->MSS<<"     sFlow->ssthresh:"<<sFlow->ssthresh<<std::endl;
...
//std::cout<<"reduceCWND  ssthresh 3   "<<"sFlow->cwnd:  "<<sFlow->cwnd<<"     sFlow->ssthresh:"<<sFlow->ssthresh<<std::endl;
}
...
MpTcpSocketBase::NewACK (uint8_t sFlowIdx, MpTcpHeader mptcpHeader, TcpOptions *opt)
{
    ...
    MpTcpSubFlow * sFlow = subflows[sFlowIdx];
//for(int i=0;i<8;i++)
//std::cout<<"a----b sfr["<<(int)i<<"]:"<<sfr[i]<<std::endl;

 //std::cout<<"newack --------- flow:"<<int(sFlow->dPort)-5000<<"sFlowIdx:"<<(int)sFlowIdx<<"     1--sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<"    "<<"sfr["<<(int)sFlowIdx<<"]:"<<sfr[sFlowIdx]<<std::endl;

    uint32_t ack = (mptcpHeader.GetAckNumber ()).GetValue();//表示收到的ack总数目，即收到的ack段的最后的值
    uint32_t ackedBytes = ack - sFlow->highestAck - 1;//表示这次收到的ack段的长度，sFlow->highestAck表示下次期待收到的ack序列号。
//std::cout<<"ackedBytes:"<<ackedBytes<<"    ack:"<<ack<<"sFlow->highestAck:"<<sFlow->highestAck<<std::endl;
...
//std::cout<<"     2--sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<std::endl;       
OpenCWND (sFlowIdx, ackedBytes);//std::cout<<" OpenCWND (sFlowIdx, ackedBytes)"<<std::endl;
        NotifyDataSent ( GetTxAvailable() );
        SendPendingData ();
//std::cout<<"NewACK  sFlowIdx:"<<(int)sFlowIdx<<"     sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<"    "<<"   sfr[sFlowIdx]:"<<sfr[sFlowIdx]<<std::endl;
    }else if( (useFastRecovery == true) || (m_algoPR == F_RTO && frtoStep == Step_4) )
    {
        ...
        std::cout<<"     3--sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<std::endl;  
        OpenCWND (sFlowIdx, 0);//std::cout<<" OpenCWND (sFlowIdx, 0)"<<std::endl;
        SendPendingData ();
    }
        if(sFlow->rtt->GetCurrentEstimate().GetSeconds()!=0)
        {
         // std::cout<<"newack qian  sFlowIdx:"<<(int)sFlowIdx<<"    sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<"    "<<"sfr[sFlowIdx]:"<<sfr[sFlowIdx]<<std::endl;
          sfr[sFlowIdx]=std::min(sfr[sFlowIdx],sFlow->rtt->GetCurrentEstimate().GetSeconds());
         // std::cout<<"newack sFlowIdx:"<<(int)sFlowIdx<<"    "<<"sfr[sFlowIdx]:"<<sfr[sFlowIdx]<<std::endl;
        }
}
...
//std::cout<<"IsDuplicatedAck sFlow->ssthresh "<<sFlow->ssthresh<<std::endl;
                        }else
                        {
...
//std::cout<<"DupAck (uint8_t sFlowIdx, DSNMapping * ptrDSN)  sFlowIdx:"<<(int)sFlowIdx<<"    flow："<<sFlow->dPort-5000<<"       Second:"<<Simulator::Now().GetSeconds()<<" reduceCWND (sFlowIdx)"<<"   sFlow->cwnd"<<sFlow->cwnd <<"     sFlow->rtt->GetCurrentEstimate().GetSeconds():"<<sFlow->rtt->GetCurrentEstimate().GetSeconds()<<std::endl;
...
MpTcpSocketBase::LookupByAddrs (Ipv4Address src, Ipv4Address dst)
{
    ...
    if(! (sFlowIdx < m_maxSubFlowNumber) )
    {
      if(m_connected == false && subflows.size()==1)//没有新的子流链接成功
      {
          sFlowIdx = 0;
      }
      else
...
//std::cout<<"i am z 1"<<" routeId "<<sFlow->routeId<<" subflow.size: "<<subflows.size()<<" seconds: "<<Simulator::Now().GetSeconds()<<std::endl;
//std::cout<<"i am z 2"<<" (int)sFlowIdx "<<(int)sFlowIdx<<" subflow.size: "<<subflows.size()<<" seconds: "<<Simulator::Now().GetSeconds()<<std::endl;
    return sFlowIdx;
}

double
MpTcpSocketBase::SlowStartCtrol(uint8_t sFlowIdx)
{
        MpTcpSubFlow * sFlow = subflows[sFlowIdx];
        double   increment = 0;
       double   cwnd      = sFlow->cwnd;
...
double b =0;*/
        ...
        if(sFlow->rtt->GetCurrentEstimate().GetSeconds() >= sfk[sFlowIdx])
            {//std::cout<<"m_algoSS:"<<m_algoSS<<std::endl;
                switch(m_algoSS)
                {
                     case Slow_Start_RR:

                          increment = 1; std::cout<<"cwnd为："<<sFlow->cwnd<<"  Slow_Start_RR increment:"<<increment<<std::endl;
                          break;
                     case Slow_Start_MyAlgo:

                         a = (1.0*sFlow->rtt->GetCurrentEstimate().GetSeconds() - sfk[sFlowIdx])/(sFlow->rtt->GetCurrentEstimate().GetSeconds()*1.0);

                               increment = pow(cwnd,-a);
                            
//increment=0.16;
                                std::cout<<"Slow_Start_MyAlgo increment:"<<increment<<std::endl;
...
void 
MpTcpSocketBase::InitialRtt(uint16_t i)
{
   
        switch(i){
           case 0:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 1:sfp[i].rtt=0.15;
                 sfr[i]=0.15;break;
           case 2:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 3:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 4:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 5:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 6:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
           case 7:sfp[i].rtt=0.15;
                  sfr[i]=0.15;break;
        }
...
MpTcpSocketBase::OpenCWND (uint8_t sFlowIdx, uint32_t ackedBytes)
{
    ...
    sfk[sFlowIdx] = ( buffersize + bottleneckBandwidth * sfr[sFlowIdx]) / (2.0 * bottleneckBandwidth );
    ...
    if ( sFlow->phase == DSACK_SS )
    {
        ...
        {
            increment = SlowStartCtrol(sFlowIdx);
            normalCC  = false;
        ...
        }else
        {
            ...
            sFlow->savedCWND = 0;//std::cout<<"Congestion Control (DSACK Slow Start) reached"<<std::endl;
            sFlow->phase = Congestion_Avoidance;
        }
...
//std::cout<<"alpha / totalCwnd:"<<alpha / totalCwnd<<"alpha"<<alpha<<"totalCwnd"<<totalCwnd<<"   1.0 / cwnd:"<<1.0 / cwnd<<std::endl;
    ...
    if ((totalCwnd + increment <= remoteRecvWnd) && (no_ll == true)){
        sFlow->cwnd += increment;std::cout<<"OpenCWND flow:"<<int(sFlow->dPort)-5000<<"sFlowIdx:"<<(int)sFlowIdx<<"       cwnd："<<sFlow->cwnd<<"   increment:"<<increment<<"     Second:"<<Simulator::Now().GetSeconds()<<"no_ll"<<no_ll<<std::endl;
    }
    double rtt = sFlow->rtt->GetCurrentEstimate().GetSeconds();
...
void 
MpTcpSocketBase::SetSlow_StartAlgo (Slow_StartAlgo_t ssalgo)
{
        NS_LOG_FUNCTION_NOARGS ();
    m_algoSS = ssalgo;
//std::cout<<"SetSlow_StartAlgo  ssalgo:"<<ssalgo<<std::endl;
}

void 
MpTcpSocketBase::SetSlow_StartSubflowAlgo (Slow_StartSubflowAlgo_t llalgo)
{
        NS_LOG_FUNCTION_NOARGS ();
    m_algoLL = llalgo;
//std::cout<<"SetSlow_StartAlgo  ssalgo:"<<ssalgo<<std::endl;
...
void
MpTcpSocketBase::SetBottleneckBandwidth (uint32_t Bandwidth)
{
         bottleneckBandwidth = Bandwidth;
}

void
MpTcpSocketBase::SetBufSize (uint32_t bufsize)
{
        NS_LOG_FUNCTION_NOARGS ();
    buffersize = bufsize;
...
//}
//
//} // namespace ns3m_slowstartsubflowAlgom_slowstartsubflowAlgo
